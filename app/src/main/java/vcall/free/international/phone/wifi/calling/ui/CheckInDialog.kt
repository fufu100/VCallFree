...
class CheckInDialog(context: Context,val callback:(point:Int) -> Unit): Dialog(context, R.style.CustomDialog) {
    companion object{
        val points = intArrayOf(20,50,100,200,300,400,500)
    }
    ...
    init {
        ...
        println("CheckInDialog $today $yesterday $lastCheckDay $consecutive_check")
    ...
    fun checkIn(){
        if(lastCheckDay == today) {
            ...
            callback(points[consecutive_check+1])
            dismiss()
        }
    }
}